...
> 2026-06-08 同步：U0/U1/U2/U3/U4 完成；**U5 预算科目树完成（6 接口：树查询含金额后序汇总 / pg_trgm 模糊搜索 / 新增子级 level·is_leaf 不变量 / 比对 / 确认新增 / 删除；13 本地 PG 集成测试覆盖 T-1..T-13）**。U4+U5 齐备 → **U6 预算模板导入解锁（关键路径下一站）**。U14 前端业务集成解锁面随各业务页推进。
...
| 就绪 | 进行中 | 阻塞 | 已完成 |
|---|---|---|---|
| **U6 ← U4,U5**（关键路径，下一步） | — | U7 ← U6 | U0 工程脚手架；U1 数据库基线（23 表+6 角色） |
| | | U8 ← U7；U9 ← U8 | U2 认证/权限基座（审查 + 本地集成测试） |
| | | U13 ← U6,U8 | U3 前端外壳（Router/守卫/apiClient·11 测试） |
| | | U10/U11/U12 ← U9 | U4 组织/项目组/用户角色（18 接口，17 集成测试） |
| | | U14 ← U4..U13（契约） | **U5 预算科目树（6 接口，13 集成测试）** |
...
| B 预算科目与审批 | U6 | 预算模板导入（FastExcel + 校验） | `import` | FP-3 | 全栈 | U4,U5 | — | 硬 | B1 | M | 🔜 就绪 |
