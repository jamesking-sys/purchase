...
> 2026-06-11 同步：**代码审查收口（commit `005f344`）**——`/code-review` 发现 5 项已修：盘点 `saveActuals` 加 FOR UPDATE 锁串行化 confirm（守 INV-2）/ `GlobalExceptionHandler` 补 `MethodArgumentTypeMismatchException→40001` / 前端文件 input 选完重置 value / ComparePage 路径去重 / InboundPage `Number(v)` NaN 兜底；配套测试后端 Stocktake 13、BudgetVsActual 7（新增非数字参→400）、前端 37 全绿。两项保留：盘点 gain/loss 符号（U12 §5.2 已知，待产品定冻结）、ILIKE 通配转义（可选）。
> 2026-06-11 同步：**测试阶段产物（tkxm-test）落盘 `docs/test/`**——测试方案 `procurement-test-plan.md`（范围/类型/环境/排期/准入准出/缺陷/风险）、功能用例 `procurement-test-case.md`（14 模块 **99 条**用例：P0 17 / P1 52 / P2 30，双源推导映射各详设 T-x/AC-x，覆盖正常/边界/异常/权限/状态机/并发/数据校验/一致性）、冒烟用例 `procurement-smoke-test.md`（13 条端到端主线串联：组织→预算→审批→采购入库→库存→领用出库→盘点→对比→工作台）。供测试执行；与开发侧 14 后端集成测试 + 18 前端测试白盒底座互补。
