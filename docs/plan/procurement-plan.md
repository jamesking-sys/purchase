...
> 2026-06-08 同步：U0/U1 已落地；**U2 认证/权限基座代码 + 单测完成（编译通过、单测绿；集成测试 Docker 门控待 CI），进入「进行中·待审查」**。U2 完成后 **U4 ∥ U5 解锁**（首个并行波次），U3 前端外壳亦可基于登录契约并行。

| 就绪 | 进行中 | 阻塞 | 已完成 |
|---|---|---|---|
| U3 ← U0,U2(契约) | **U2 认证/权限基座**（待审查/CI） | U6 ← U4,U5；U7 ← U6 | U0 工程脚手架 |
| **U4 ← U2**（关键路径） | | U8 ← U7；U9 ← U8 | U1 数据库基线（23 表 + 6 角色种子） |
| **U5 ← U2**（协同关键路径） | | U13 ← U6,U8 | |
| | | U10/U11/U12 ← U9 | |
| | | U14 ← U4..U13（契约） | |
...
| 基建 | U2 | 认证 / 权限基座（Sa-Token + 角色校验） | （登录态） | FP-1(部分) | 后端 | U1 | — | 硬 | A2 | M | 🟡 代码完成·待审查 |
| 前端 | U3 | 前端外壳（Semi UI 布局/导航/路由） | 全局导航 | — | 前端 | U0 | U4,U5 | 软·契约(登录) | A2 | M | 🔜 就绪 |
| A 基础与权限 | U4 | 组织/项目组/用户角色管理 | `org` | FP-1 | 全栈 | U2 | U5 | 硬 | A1,A2 | M | 🔜 就绪 |
